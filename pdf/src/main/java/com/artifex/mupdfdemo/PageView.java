...
public abstract class PageView extends ViewGroup {
	...
	protected     ArrayList<ArrayList<PointBean>> mDrawing;
	...
	private static OnMarkClickListener onMarkClickListener;
	private static OnDrawFinishedListener onDrawFinishedListener;
	...
	public void setPage(int page, PointF size) {
		...
		if (mSearchView == null) {
			mSearchView = new View(mContext) {
				...
				protected void onDraw(final Canvas canvas) {
					...
					if (mDrawing != null && mDrawing.size() != 0) {
						if (mDrawing.size() == pointCounts) {
							View view = null;
							for(int index = getChildCount();index > 0;index --){
								view = getChildAt(index);
								if(view != null && view instanceof WaveView){
									removeView(view);
								}
							}
						}
						...
						Iterator<ArrayList<PointBean>> it = mDrawing.iterator();
						while (it.hasNext()) {
							ArrayList<PointBean> arc = it.next();
							if (arc.size() >= 2) {
								Iterator<PointBean> iit = arc.iterator();
								p = iit.next();
								float mX = p.getPointF().x * scale;
								float mY = p.getPointF().y * scale;
								path.moveTo(mX, mY);
								while (iit.hasNext()) {
									p = iit.next();
									float x = p.getPointF().x * scale;
									float y = p.getPointF().y * scale;
								...
								waveView = new WaveView(mContext);

								waveView.layout((int) (x - 100), (int) (y - 100), (int) x + 100, (int) y + 100);
								waveView.setPointF(p.getPointF());
								waveView.setDuration(5000);
								waveView.setStyle(Paint.Style.FILL);
								waveView.setColor(Color.RED);
								waveView.setInterpolator(new LinearOutSlowInInterpolator());

								waveView.setTag(p.getTag());
			...
			mSearchView.post(new Runnable() {
				@Override
				public void run() {
					onDrawFinishedListener.onFinished();
				}
			});
		}
	...
	public static void setOnMarkClickListener(OnMarkClickListener l) {
		onMarkClickListener = l;
	}

	public static void setOnDrawFinishedListener(OnDrawFinishedListener listener) {
		onDrawFinishedListener = listener;
	}
	...
	public void startDraw(float x, float y, String tag) {
		float scale = mSourceScale * (float) getWidth() / (float) mSize.x;
		float docRelX = (x - getLeft()) / scale;
		float docRelY = (y - getTop()) / scale;
		if (mDrawing == null)
			mDrawing = new ArrayList<ArrayList<PointBean>>();

		ArrayList<PointBean> arc = new ArrayList<PointBean>();
		arc.add(new PointBean(new PointF(docRelX, docRelY), tag));
		mDrawing.add(arc);
	...
	public void continueDraw(float x, float y, String tag) {
		float scale = mSourceScale * (float) getWidth() / (float) mSize.x;
		float docRelX = (x - getLeft()) / scale;
		float docRelY = (y - getTop()) / scale;

		if (mDrawing != null && mDrawing.size() > 0) {
			ArrayList<PointBean> arc = mDrawing.get(mDrawing.size() - 1);
			arc.add(new PointBean(new PointF(docRelX, docRelY), tag));
			mSearchView.invalidate();
		}
	}
	...
	protected PointF[][] getDraw() {
		...
		for (int i = 0; i < mDrawing.size(); i++) {
			ArrayList<PointBean> arc = mDrawing.get(i);
